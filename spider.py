...
# 影片图片
findImgSrc = re.compile(r'<img.*src="(.*?)"', re.S)  # 让换行符包含在字符串中
# 影片的片面
...
def askURL(url):
    ...
    # 用户代理，告诉豆瓣服务器，我们是什么类型的机器（本质上是告诉浏览器，我们可以接收什么水平的文件）
